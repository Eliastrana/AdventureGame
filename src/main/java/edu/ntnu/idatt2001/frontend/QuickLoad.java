    ...
    public static HBox savedGamesContainer() throws IOException {
        ...
        for (File file : listOfFiles) {
            if (file.isFile() && count < 3) { // Only process the first three files
                ...
                Text savedGameCharacter = new Text(SaveFileReader.getName(file.getPath()));
                savedGameCharacter.setStyle("-fx-fill: #ffffff");
                savedGameCharacter.setId("savedGameText");

                Text savedGamePath = new Text(SaveFileReader.getPath(file.getPath()));
                savedGamePath.setId("savedGameText");
                savedGamePath.setStyle("-fx-fill: #ffffff");

                Text savedGameCurrentPassage = new Text(SaveFileReader.getLastSeenPassage(file.getPath()));
                savedGameCurrentPassage.setId("savedGameText");
                savedGameCurrentPassage.setStyle("-fx-fill: #ffffff");
